...
def predict():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('index.html',symptoms_dict=symptoms_dict, symptoms=symptoms, pred_disease=pred_disease, desc=desc, pre=pre[0], med=medications,
                                   die=diets, wrkout=wrkout)

        except KeyError:
            error_message = "The entered symptoms did not match any known disease. Please check your input and try again."
            return render_template('index.html',symptoms_dict=symptoms_dict, symptoms=symptoms, error=error_message)
